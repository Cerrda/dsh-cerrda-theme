...
const vendorPath = join(root, 'src', 'vendor', 'number-flow.inline.js')
...
// Vendored number-flow (window.__CERRDA_NUMBER_FLOW__) — generated by
// scripts/vendor-number-flow.mjs; must exist before the theme source runs.
const vendor = readFileSync(vendorPath, 'utf8').replace(/\r\n/g, '\n')
const vendorMarker = 'window.__CERRDA_NUMBER_FLOW__ = (function () {'
if (!vendor.includes(vendorMarker)) {
  throw new Error(`build: unexpected vendored number-flow shape in ${vendorPath}; regenerate with scripts/vendor-number-flow.mjs`)
}
src = vendor + '\n' + src
...
const stylesMarker = `disposers.push(styles.insert(THEME_CSS));`
